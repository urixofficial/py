...
class MainWindow(QWidget):
	...
	def showTable(self, dbPath, tableName):
	...
	def showTableContetnts(self, dbPath, tableName, sort=1):
		...
		logger.debug([editor for editor in self.dbTable.horizontalHeader()._editors])
		...
		if filterString: sqlString += f' WHERE {filterString}'
	...
	def editRow(self, dbPath, tableName):
		...
		if dialog.exec_():
			...
			# внесение изменений в БД
			cur.execute(f'UPDATE OR IGNORE {tableName} SET ({headers}) = ({values});')
			con.commit()
